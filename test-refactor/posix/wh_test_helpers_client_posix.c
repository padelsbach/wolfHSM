 ...
 */
/*
 * test-refactor/wh_test_helpers_client_posix.c
 *
 * POSIX client-side init. Wires the client onto the shared
 * mem-transport buffers published by the server helper; the
 * port's server thread is responsible for pumping requests.
 */
...
#include "wh_test_helpers_client_posix.h"


/* Client-side transport state (buffers are shared with the
 * server via whTestHelperPosix_Server_GetTransportConfig) */
