 ...
 */
/*
 * test-refactor/wh_test_helpers_server_posix.h
 *
 * POSIX server-side init helper. Allocates the NVM, crypto,
 ...
 * client helper can wire its side onto the same
 * request/response buffers.
 */

#ifndef WH_TEST_HELPERS_SERVER_POSIX_H_
#define WH_TEST_HELPERS_SERVER_POSIX_H_
 ...
 */
int whTestHelperPosix_Server_Init(whServerContext* server);

/*
 * Tear down the server context plus backing state. Matches
 * Server_Init one-for-one.
 */
int whTestHelperPosix_Server_Cleanup(whServerContext* server);

/*
 * Returns the shared mem-transport config (buffers + sizes).
 * Used by the POSIX client helper to wire its side onto the
 * same buffers the server publishes through.
 */
whTransportMemConfig* whTestHelperPosix_Server_GetTransportConfig(void);

#endif /* WH_TEST_HELPERS_SERVER_POSIX_H_ */
