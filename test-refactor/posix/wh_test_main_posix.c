 ...
 */
/*
 * test-refactor/wh_test_main_posix.c
 *
...
static void* _serverThread(void* arg)
{
    ...
    rc = whTestHelperPosix_Server_Init(&_server);
    if (rc != 0) {
        _serverRc = rc;
    ...
    if (rc != 0) {
        _serverRc = rc;
        (void)whTestHelperPosix_Server_Cleanup(&_server);
        _signalServerReady();
        return NULL;
    }
...
static void* _clientThread(void* arg)
{
    ...
    rc = whTestHelperPosix_Client_Init(&_client);
    if (rc != 0) {
        _clientRc = rc;
        return NULL;
    }
    ...
    (void)whTestHelperPosix_Client_Cleanup(&_client);
    return NULL;
}
